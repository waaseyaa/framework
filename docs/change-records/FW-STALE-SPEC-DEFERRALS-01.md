...
1. Scan `docs/specs/**/*.md` body prose only. Blank `<!-- Spec reviewed -->`
   blocks and fenced examples so append-only review logs and samples stay
   dated-wrong on purpose.
2. Match a narrow deferral vocabulary. Strong phrases (`blocked on`,
   ...
   before `#NNNN`. `deferred to` / `pending` / `until` / `after` / `once`
   must be adjacent to the hash so "deferred to a follow-up PR (mission
   #1257)" and positional "after routers (#1129)" do not fire. `after #N`
   used as provenance ("the listener uses after #1856") is dropped.
3. Drop a match when its clause contains a past-tense verb. Query-string
   `?page=` is not a sentence boundary.

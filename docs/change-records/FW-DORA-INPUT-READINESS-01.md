...
| Consumer production promotion | `jonesrussell/waaseyaa-infra` `promote.yml` | Protected Environment `production`. Manual `workflow_dispatch` with one target, exact 40-character SHA, and reason. Latest run `33474562592`: Promote `northway` to production at `0888d56e0114…` on 2026-09-01T05:41Z. Targets include `minoo`, `giiken`, `northway`, `jgs`, `fnpi`, `goformx`, `oiatc`, `rhtcircle`, `sagamokstrong`, `spot-the-ai`. |
...
| Lead time for changes | PR `github_created_at` → `merged_at` (pre-production). Framework merge → tag/Packagist (publication lag). | First commit → consumer production promote. Framework SHA → `MINOO_REF`/target pin → `promote.yml`. |

...
### Changed-line coverage repair

Hosted CI measured104/569 changed executable lines (18.28%). Existing CLI integration tests explicitly asserted verifier/report/containment behavior but their coverage metadata named only the adapter. Correcting that attribution yielded72.23% in focused coverage, so six additional discriminating refusal/redaction cases cover missing targets, retired/unrecorded ownership, oversized input, markerless guidance, and missing/corrupt canonical sources. Four focused files now pass55tests/187assertions; PCOV1.0.12 on matching PHP8.5.8 API20250925 reports481/569 changed lines (84.53%,required80%). Execution0.775s162MB, report0.922s. Existing extension loaded for this command only; no configuration change or full-suite run. Remaining uncovered branches include cwd/read failures and defensive malformed-input branches, not claimed complete coverage. Hosted exact-head measurement remains authoritative.

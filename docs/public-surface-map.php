...
return [
    ...
    // eligibility policy for the crawler-facing surfaces. SitemapPath,
    // DiscoveryConfigurationException, and NonPublicEntityTypes are concrete
    // finals and so are not found by the parity gate's automatic discovery, but
    // they are respectively the return type, the failure type, and the declared
    // floor of @api interfaces, and are therefore public surface; they are listed
    // deliberately so a removal is caught as removal-without-deprecation.
    // DiscoverySurfaceGovernanceTest keeps this list and the @api annotations in
    // lockstep in BOTH directions, so a new Discovery type cannot ship ungoverned
    // and an existing entry cannot be quietly dropped.

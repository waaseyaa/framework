...
<!-- Spec reviewed 2026-08-10 - #2327 configured-community principal convergence: CommunityMiddleware normalizes the resolved authoritative community onto `_community_id` before FieldReadContextMiddleware constructs the immutable principal. Explicit route/session precedence is preserved; a configured fixed-community route now carries the same scope as storage/controllers, and an inactive context remains null. -->

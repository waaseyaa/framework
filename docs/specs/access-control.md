...
<!-- Spec reviewed 2026-07-02: WP5 addresses the sibling entity-level surface for `group`/`group_type` — JSON:API CRUD (JsonApiController's isAllowed() gate) already denies every account, including admins, because no AccessPolicyInterface is registered for either type. Rather than add a policy, the PARK decision de-advertises both types (`discoverable: false`) and documents the deny-by-default as intentional (`packages/groups/README.md` "API exposure (parked)", `GroupApiSurfaceParkedTest`). Full route suppression is deferred to the planned entity-type `api:` exposure flag (#1871). This paragraph's query-layer Neutral-admits-a-row analysis is unaffected — it concerns list/count access-checked queries, not the single-entity CRUD path this park note covers. -->

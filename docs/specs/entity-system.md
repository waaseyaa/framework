...
|----------|-------|------|--------------------------|
| `getRepository()` | `EntityRepository` | revisions, optimistic locking (`SaveContext`), save-time validation, two-axis translation, language fallback, `saveMany()`/`UnitOfWork`; **also owns an access-checked `getQuery()`** (C-22) | `ai-*`, `media`, `attachment`, the optimistic-locking branch of `JsonApiController` |
...
**Access-checked query parity (C-22).** As of the C-22 prerequisite, `EntityRepository::getQuery(): EntityQueryInterface` builds the **same** access-checked `SqlEntityQuery` as `SqlEntityStorage::getQuery()`: same query construction, the same lazily-resolved `EntityAccessHandler` (threaded by the kernel's `EntityTypeManagerFactory` via the same resolver both engines share), the same fail-closed `setAccount()` contract (unbound account → `MissingQueryAccountException` on `execute()`), and an id-keyed entity loader matching `SqlEntityStorage::loadMultiple()`'s shape so the per-row `check($entity, 'view', $account)` filter is identical. Two optional trailing `EntityRepository` constructor params carry the handler (`?EntityAccessHandler $accessHandler`, `?\Closure $accessHandlerResolver`). Parity is locked by `tests/Integration/PhaseN/EntityQueryAccessCheck/RepositoryStorageQueryParityTest.php`. This is the additive step that unblocks migrating the ~20 `getStorage()->getQuery()` consumers off the legacy engine (inventory: `docs/notes/c22-consumer-inventory.md`); no consumers were migrated and neither engine was removed.

**Canonical path for new code: `EntityRepository` (`getRepository()`)** — it is the pipeline mandated by `.claude/rules/entity-storage-invariant.md` and the only path that carries events + revisions + validation + locking, and it now also carries the access-checked `getQuery()`. `getStorage()` remains for the lower-level `EntityStorageInterface`. There is **no** `StorageRepositoryAdapter` — earlier rule files named one, but convergence onto a single engine (making `SqlEntityStorage` a thin facade over `EntityRepository`, or extracting a shared core) is a **deferred future item**, intentionally not attempted while both accessors back the live production write path.

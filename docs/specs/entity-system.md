...
#### Two save engines (C-22 WP3: legacy engine now has zero production consumers)

The framework historically exposed **two entry points that both persisted entities in production** — `getRepository()` (`EntityRepository`, canonical) and `getStorage()` (`SqlEntityStorage`, legacy). C-22 WP0–WP2 gave `EntityRepository` an access-checked `getQuery()` and migrated every `getStorage()->getQuery()` consumer onto it; **C-22 WP3 completed the migration** by moving every remaining `getStorage()`-based `load()`/`loadMultiple()`/`create()`/`save()`/`delete()`/`loadByKey()` callsite onto the equivalent `getRepository()` method (`find()`/`findMany()`/`findBy([])`/`create()`/`save()`/`delete()` — a bounded `getQuery()` + `find()` for the `loadByKey()` cases, since the repository has no direct key-lookup equivalent).

| Accessor | Class | Status after C-22 WP3 |
|----------|-------|------------------------|
| `getRepository()` | `EntityRepository` | **The only production persistence path.** Carries revisions, optimistic locking (`SaveContext`), save-time validation, two-axis translation, language fallback, `saveMany()`/`UnitOfWork`, the access-checked `getQuery()`, and (as of WP3) `create()` — field-default application + hydration, shared with the legacy engine via `EntityInstantiator::applyFieldDefinitionDefaults()` so the two never drift on defaulting behavior. |
| `getStorage()` | `SqlEntityStorage` | **No production callers remain.** `create()`/`load()`/`loadByKey()`/`loadMultiple()`/`save()`/`delete()` are confirmed dead by the dead-code gate (baselined in `phpstan-dead-code-baseline.neon`, pending deletion in C-22 WP4). `EntityStorageFactory` (the storage-only construction helper) is likewise unreferenced. The interface/accessor themselves are not yet removed — C-22 WP4 deletes `SqlEntityStorage`, `EntityStorageFactory`, and (if nothing else needs it) `EntityStorageInterface`/`EntityTypeManager::getStorage()` once the WP1 behavior-identity harness confirms there is nothing left to lose. |

**Behavior changes from WP3 (documented, not silent):**
- **JSON:API PATCH without a stated `expected_revision_id` now cuts a revision on revisionable types.** Previously this branch stayed on `getStorage()->save()` (revision-less) per an explicit contract (`kitty-specs/archive/optimistic-locking-01KTXCHY/contracts/conflict-surfaces.md` clause 14); WP3 unifies both PATCH branches onto `getRepository()->save()`, simply omitting the `SaveContext::withExpectedRevisionId()` guard when no expectation is stated. See `docs/specs/api-layer.md` "Conditional update — optimistic locking".
- **`SessionMiddleware`/`BearerAuthMiddleware` constructors narrowed from `EntityStorageInterface` to `EntityRepositoryInterface`** — the kernel now passes `getRepository('user')` instead of `getStorage('user')`.
- **`NoteIngester`/`OidcClientSeeder`/`OidcClientLookup` constructors narrowed to a single `EntityRepositoryInterface` param** (previously accepted `EntityStorageInterface`, or both).

**Canonical path for new code: `EntityRepository` (`getRepository()`)** — mandated by `.claude/rules/entity-storage-invariant.md`. `getStorage()` still exists (interface + kernel wiring untouched by WP3) but has no production callers; treat it as scheduled for removal, not as a supported alternative. There is **no** `StorageRepositoryAdapter` — earlier rule files named one; C-22 WP4 (delete `SqlEntityStorage` outright, now that it is confirmed dead) supersedes that deferred-convergence idea.

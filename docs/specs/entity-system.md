...
identity before the shared registry freezes. Duplicate registration is
idempotent only for the same validator class; a competing validator or any late
registration is refused. `ConfigContentHasher` runs that package-owned semantic
validation after structural schema validation and before it derives authored or
effective identities, so invalid domain values cannot acquire trusted hashes.
The package's declared configuration contract version binds responsibility for
those semantics and must advance when their compatibility changes.

...
`ClaudeClientTransformer` and `CodexClientTransformer` share
`AbstractPerSkillClientTransformer`: concise always-loaded guidance plus
one on-demand skill directory per canonical inventory entry. Claude keeps
guidance in `.claude/CLAUDE-WAASEYAA.md` so the consumer's hand-authored
root `CLAUDE.md` is not overwritten. Codex uses root `AGENTS.md` for
guidance and `.agents/skills/waaseyaa-<id>/SKILL.md` for skill bodies
(#2660 Part B). The other five clients use
`AbstractSingleFileClientTransformer` — one consolidated file per project,
with deterministic warnings when `--features=skills` is requested for a
...
| Surface | Always loaded? | Generated by `bimaaji:install` today? |
|---|---|---|
| Guidelines | Yes — concise cross-cutting instructions | Yes — `AGENTS.md`, `.claude/CLAUDE-WAASEYAA.md`, or other client guidance paths |
| Skills | On demand — detailed subsystem bodies | Yes for `claude` and `codex` only (per-skill directories) |
| MCP configuration | Client-specific | No — owned by #2663 |

Single-file clients fold skill bodies into their one guidance file when
installed; `ClientCapabilityDiagnostics` prints a warning rather than
silently omitting coverage. Requesting `mcp_configuration` in
`--features` prints a warning that MCP output is not generated here.

Every managed region carries a source inventory provenance marker
(`<!-- waaseyaa:bimaaji:source-inventory sha256=… -->`) derived from the
canonical `SKILL.md` bytes. Re-running the install with the same packaged
skill set regenerates identical output byte-for-byte.

...
A second scan — **rule PL008** — covers the complementary blind spot: quoted `'Waaseyaa\\Ns\\...'` string literals in Layer-0 `src/` files whose namespace maps to a higher-layer package. PL005 misses these because they are not `use` imports; the CLAUDE.md "Layer discipline for imports" rule deliberately permits string constants for cross-layer attribute scanning (where `::class` would be an upward import violation), but each legitimate entry must be allowlisted with a rationale in `tools/package-layers-string-literal-baseline.txt`. New violations fail CI (fail-on-new); the self-test in `bin/check-package-layers-pl008-self-test` (wired into `composer verify`) asserts both fire-on-violator and green-after-allowlist.

**PL008 sub-pattern (b) — inline `::class` tokens (WP7 audit remediation):** the original PL008 regex required a leading quote character, so it only caught QUOTED string-literal FQCNs (sub-pattern (a), two backslashes per namespace separator in raw file text, e.g. `'Waaseyaa\\Node\\Foo'`). An inline, unquoted, fully-qualified `\Waaseyaa\<Ns>\...::class` token with no `use` import (one backslash per separator, since this is real PHP syntax rather than a string literal) was invisible to BOTH PL005 (no `use` statement to scan) and the original PL008 regex (no leading quote) — the exact shape found in `packages/foundation/src/Http/ControllerDispatcher.php`'s `!\class_exists(\Waaseyaa\SSR\SsrPageHandler::class)` optional-dependency probe. `$scanStringLiteralFqcns` in `bin/check-package-layers` now matches both sub-patterns under the same PL008 rule ID (same finding/baseline/emit path — either shape is fixed by moving the reference to the owning higher-layer package or switching to the string-constant convention, then baselining if intentional); a negative lookbehind on the inline-`::class` regex prevents it from double-matching the second backslash of a sub-pattern-(a) quoted-literal sequence. `ControllerDispatcher.php` was fixed by using its already-declared `SSR_PAGE_HANDLER` string constant at the callsite (the constant existed but was unused pre-fix) and is baselined with a rationale. `tests/Architecture/CheckPackageLayersGateTest.php` gained fixture-driven coverage for both sub-patterns (fires on quoted literal, fires on inline `::class`, baseline suppresses a known inline-`::class` finding, no false positive for a same-or-lower-layer inline `::class` reference), extending its fixture writer to plant arbitrary raw class-body text and passing `WAASEYAA_LAYER_STRING_LITERAL_BASELINE=/dev/null` so fixture runs never fall through to the real repo baseline.
...
The `Http/Router/` built-in domain routers (`JsonApiRouter`, `SchemaRouter`, `SearchRouter`, `TranslationRouter`, `EntityTypeLifecycleRouter`, `CodifiedContextApiRouter`, `WorkflowDefinitionsApiRouter`, `BroadcastRouter`) plus the `WaaseyaaContext` request DTO are a **permanent** entry in this surface, not a staging area: the kernel owns the `DomainRouterInterface` contract (L0, public) and eagerly constructs these concretes inside `HttpKernel::dispatch()` so the JSON:API/SSE chain exists before any provider-supplied routers and the terminal `BroadcastRouter` — including on a `waaseyaa/core`-only install with no `api` ServiceProvider. They carry no `@api` tag and have no package/skeleton/public importers (the only external referent is one integration test, `tests/Integration/Phase24/SemanticSearchAccessTest`, that constructs `SearchRouter` directly — which pins them as a fixture, not a relocation target). Relocating them to L4 is net-negative and is **not** pending.
...
| Media upload | `POST /api/media/upload` | `access media` |
...
**Stored-payload `unserialize()` trust boundary (D-12):** Three Layer-0 stores deserialize without an `allowed_classes` guard — `Worker::processJob()` (`packages/queue/src/Worker/Worker.php`), `DatabaseBackend::mapRowToItem()` (`packages/cache`), and `SqlState::get()/getMultiple()` (`packages/state`). This is **intentional and cannot be tightened to `allowed_classes => false`**: all three must restore arbitrary consumer-defined object graphs. Queue messages are any `object` (no marker interface; `QueueInterface::dispatch(object)`), dispatched to `HandlerInterface::handle()` and tested with `instanceof Job`; cache `data` and state values are `mixed` and routinely hold objects (`SqlStateTest::testSerializationOfObject` pins this). A static allowlist is infeasible because the legitimate class set is open-ended and owned by downstream consumers. **Threat model:** all three payloads are serialized *by this application* into server-controlled SQLite tables; there is no request-derived write path, so object-injection requires a pre-existing DB compromise (at which point the gadget surface is moot). **Deferred hardening (tracked tech-debt, not done in the audit pass):** integrity-sign stored payloads with an HMAC keyed off `app.secret` and verify before `unserialize()`. This is a storage-format migration (existing unsigned payloads must be tolerated or re-written) plus key management, hence out of scope for a minor remediation. Callsites carry an inline `D-12` docblock pointing here.
...
- **`ServiceProvider` has no `$dispatcher` property**: Event subscriber registration must resolve the dispatcher via `$this->resolve(\Symfony\Contracts\EventDispatcher\EventDispatcherInterface::class)` (the ONLY FQCN `ProviderRegistryKernelServices::get()` serves it under) and check `instanceof Waaseyaa\Foundation\Event\EventDispatcherInterface` before calling `addListener()`/`addSubscriber()`. Resolving the foundation FQCN or the Symfony *Component* FQCN instead returns null (`resolveOptional()`) or throws (`resolve()`) in a real kernel boot — no exception surfaces at the `addListener()`/`addSubscriber()` call site, the registration just silently never happens. This exact bug shipped and was found live in four providers: relationship's delete guard (#1852), and — swept in WP4 (audit-remediation batch 2026-07-01/02) — field's classification-label lifecycle subscriber, and two ai-observability telemetry providers. `bin/check-dispatcher-keys` (wired into `composer verify`, baseline at `tools/dispatcher-key-baseline.txt`) fails CI on any NEW provider `register()`/`boot()` method matching this pattern.

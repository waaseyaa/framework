...
### Identifier quoting (SQL-injection hardening — database-legacy M1+M2)

The query builder binds all **values** as parameters (never interpolated), so `$value` is never an injection vector. **Identifiers** (column / alias / table / join names) are handled in two ways:

- **Builder-owned identifier paths are quoted** via the platform's `quoteIdentifier` (cross-driver; splits a qualified `alias.column` on `.` and quotes each part; doubles embedded quotes so a reserved word like `key`/`count`/`order`, or a metacharacter-bearing name, is rendered inert): `fields()` / `addField()` columns and the `AS` alias, `join()` / `leftJoin()` `$table` and `$alias`, and the WHERE-field of `DBALUpdate` / `DBALDelete` (both the simple `Connection::update`/`delete` criteria and the `applyConditions()` QueryBuilder path). `*` in `fields()`/`addField()` is left unquoted.
- **`condition()` / `orderBy()` `$field` is a developer-supplied RAW fragment** (a column, a pre-quoted identifier, OR an expression such as `SqlEntityQuery`'s `json_extract(_data, '$.x')`). It is emitted **verbatim** and is NOT quoted — the same developer-supplied-only contract as the `join()` ON-condition. **Never pass user input as `$field`.** The entity query engine relies on this (it pre-quotes identifiers and builds `json_extract` expressions in `SqlEntityQuery::resolveField()`). Auto-quoting these two `$field` arguments is deferred to a follow-up (#1816) that adds `whereRaw()` / `orderByRaw()` seams and migrates `SqlEntityQuery` to bare identifiers.

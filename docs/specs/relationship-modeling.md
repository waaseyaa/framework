...
<!-- Spec reviewed 2026-08-30 - #2728: the referential-integrity delete guard is now effective. EntityRepository::doDelete() dispatches EntityEvents::PRE_DELETE IMMEDIATELY (inside the open delete transaction) instead of buffering it through the UnitOfWork, so RelationshipDeleteGuardListener's refusal rolls back the row, its revisions and the mutation-authority tombstone on both delete() and deleteMany(). POST_DELETE stays buffered until after commit. The previous batch-only framing was wrong in both directions. Acceptance: tests/Integration/Relationship/RelationshipDeleteGuardKernelWiringTest.php. -->
<!-- Spec reviewed 2026-08-25 - #1958: RelationshipPreSaveListener registration is idempotent per RelationshipServiceProvider instance (boot() re-entry must not stack duplicate PRE_SAVE listeners). Production-shaped kernel proof (tests/Integration/Relationship/RelationshipPreSaveListenerKernelWiringTest.php) covers exactly-once registration without manual wiring, pre-persistence rejection for missing source/target/type/endpoint, valid write PRE/POST once, validate:false as EntityValidator opt-out that does not bypass the listener, GroupMembershipService under the wired path, FrankenPHP worker-shaped fresh HttpKernel boots, and a negative control that removes the listener to show the unwired path. No RelationshipValidator / schema / data-contract change. -->
...
- **Guard timing (#2728):** the guard runs inside the delete transaction on **both** `delete()` and `deleteMany()`; its throw rolls back the row, its revisions and the mutation-authority tombstone for the whole batch. The previous "only the single-`delete()` path is guarded" framing was wrong in both directions — `delete()` opens its own `UnitOfWork` whenever a mutation authority and database are wired (which `EntityTypeManagerFactory` always does), so before #2728 the guard refused only *after* the delete had committed on the single path too. Because the guard queries the transaction's own connection, in a batch it observes rows already removed for earlier entities, so **batch order is semantically load-bearing**.
- **Known limits:** a blocked delete currently surfaces as a 500 through the API (mapping to 409 Conflict is an open follow-up). Cascade-delete semantics (auto-removing edges instead of blocking) were considered and deliberately not chosen — blocking is the fail-safe default.
- `RelationshipPreSaveListener` is registered by `RelationshipServiceProvider::boot()` on the production `PRE_SAVE` lifecycle event, so every repository relationship write is normalized and validated before persistence. Registration is idempotent per provider instance (re-entering `boot()` must not stack duplicate listeners). `save(..., validate: false)` skips Symfony `EntityValidator` only; it does **not** skip this listener — `EntityRepository` always dispatches `PRE_SAVE`. Production callers that intentionally use `validate: false` for relationship rows (`GroupMembershipService`, migration `EntityDestination`, fixture/seed helpers) must still satisfy `RelationshipValidator` (existing endpoints, type shape). Acceptance: `tests/Integration/Relationship/RelationshipPreSaveListenerKernelWiringTest.php`.

...
- **Scope: every entity type.** Endpoints are free-form `(type, id)` pairs and `RelationshipValidator` accepts any registered entity type, so the guard matches the deleted entity's own `getEntityTypeId()` + id against `from_*` and `to_*` endpoint columns. (Until 2026-07-01 the guard was hardcoded to `'node'` — and was never registered at all, so it guarded nothing in production.)
- **Wiring:** `RelationshipServiceProvider::boot()` registers it on `EntityEvents::PRE_DELETE` via the foundation `EventDispatcherInterface`. The throw (`RuntimeException` "Safe-delete blocked for {type} {id}: linked relationship IDs [...]") aborts `EntityRepository::delete()` before the row is removed. To delete a referenced entity, delete (or repoint) its relationships first; deleting a relationship entity itself is guarded only if it is in turn an endpoint of a meta-relationship.

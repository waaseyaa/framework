...
<!-- Spec reviewed 2026-08-10 - #2327 configured-community convergence: the production audit provider gives FieldReadContextMiddleware the kernel-owned active CommunityContextInterface. After explicit request/session selectors, that context scopes the immutable principal used by protected reads and authorization-query cache identity; inactive contexts remain null. -->

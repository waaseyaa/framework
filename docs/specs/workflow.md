...
The top-level M11 steady-state conformance artifact is [docs/governance/m11-steady-state-conformance-loop.md](../governance/m11-steady-state-conformance-loop.md); governed changes enter through `#999`, which is the repo-local front-door into that loop. For steady-state drift scans and C17+ logging, use [docs/governance/m11-periodic-drift-scan-protocol.md](../governance/m11-periodic-drift-scan-protocol.md) and the [M11 drift-scan log issue template](../../.github/ISSUE_TEMPLATE/m11-drift-scan-log.md).

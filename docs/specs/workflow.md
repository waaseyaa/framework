...
The top-level M11 post-execution governance baseline is [m11-post-execution-governance-bootstrap.md](../governance/m11-post-execution-governance-bootstrap.md). Governed changes enter the M11 front door through `#999`, and this workflow spec serves as the repo-local proxy surface for that path. The operating loop itself is [m11-steady-state-conformance-loop.md](../governance/m11-steady-state-conformance-loop.md), and steady-state drift scans and C17+ logging use [m11-periodic-drift-scan-protocol.md](../governance/m11-periodic-drift-scan-protocol.md) and the [M11 drift-scan log issue template](../../.github/ISSUE_TEMPLATE/m11-drift-scan-log.md).

<!-- Spec reviewed 2026-08-21 - #2478: production HTTP must not CREATE or heal attachment schema; coordinated schema:sync applies AttachmentSchema via StorageSchemaTransition. -->
# Work Surface
...
`Attachment` uses the `sql-blob` storage backend (the framework default — no `primaryStorageBackend` override). Sql-blob still has no generic `#[Field]` column builder (`SqlColumnSchemaBuilder` is sql-column only). Attachment-specific columns and indexes (`parent_entity_type`, `parent_entity_id`, `is_active`, `created_at`, `updated_at`, and the composite/partial indexes) are a coordinated `#[StorageSchemaTransition]` on `Attachment` (`AttachmentSchema`). `install:init` / `db:init` / `schema:sync` apply that transition through `SqlSchemaHandler`. Production HTTP must not CREATE or heal this table (#2478): the kernel fail-closes missing registered entity tables with `[S1-DB106]` before provider boot, and `AttachmentServiceProvider::boot()` materializes schema only in local/development. Local convenience boots and schema:sync remain self-healing.

`AttachmentSchema` (`packages/attachment/src/Schema/AttachmentSchema.php`) is the canonical provider of those columns and indexes. `ensureTable()` still converges a legacy base-only table (pre-transition installs) by additively adding missing columns/indexes rather than no-op'ing.

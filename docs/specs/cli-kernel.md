...
<!-- Spec reviewed 2026-09-05 - #2833: scaffold:auth resolves upstream auth UI sources, in order, from the project's packages/admin/app, an installed waaseyaa/framework aggregate, the package-owned mirror shipped inside the installed waaseyaa/cli package (packages/cli/resources/auth-ui, kept byte-identical to packages/admin/app by bin/sync-cli-auth-ui-resources), or a loaded-package-local fallback anchored on the CLI class's own file location. This covers ADR-004 direct-package consumers that omit waaseyaa/framework; the previously documented sibling-directory guess is removed. Canonical ownership contract: auth-consumer-extensions.md. -->

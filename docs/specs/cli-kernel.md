...
<!-- Spec reviewed 2026-08-29 - #2644: `site:doctor` joins the boot-free command seam. `SiteServiceProvider::siteDoctorCommand()` is now the single definition of the command, shared by ordinary provider discovery and by `ConsoleKernel::handle()`'s pre-boot branch, exactly as `ConfigCacheDbAuditServiceProvider::dbInitCommand()` is for `db:init`. The doctor reads only the filesystem, so nothing is lost; what is gained is that verification stops being a write. Ordinary CLI boot reaches `AbstractKernel::bootDatabase()` before every restricted-discovery guard, so verifying an uninitialized project created a zero-table `storage/waaseyaa.sqlite` plus `-wal`/`-shm` sidecars and then misreported the missing site contract as an inactive configuration generation. Command name, description, options, and exit codes are unchanged. Acceptance: ConsoleKernelTest::siteDoctorDiagnosesWithoutBootingOrCreatingTheDatabase, SiteDoctorIsReadOnlyTest. -->

...
The identity bound is `AccountInterface::id()`, the only identity claim guaranteed stable across requests. `AuthorizationPrincipalInterface::claimsGeneration()` deliberately rotates, so keying on it would silently break a principal's own replays after an ordinary claims refresh. An integer uid and its digit-string form canonicalize to the same partition, matching how the rest of this surface (`SaveContext::withActorUid()`, the audit record) already treats them; a non-numeric identifier is bound verbatim under a distinct prefix. Namespace components are NUL-separated, so no entity type, bundle, or principal id can be re-partitioned by a colliding concatenation.

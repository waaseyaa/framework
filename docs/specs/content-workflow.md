...
**Forward drafts always create a revision** (same panel, finding B): in the guard's state-changing-update path, a save entering a `default_revision: false` state on an entity with a published pointer forces `setNewRevision(true)`, overriding the bundle opt-out (`NodeType new_revision: false` / entity-type `revisionDefault: false`) and even an explicit earlier `setNewRevision(false)`. Precedence: the bundle opt-out governs ordinary NON-state-changing edits; state-changing forward drafts always revision — otherwise an opt-out bundle's raw forward draft updates the CURRENT revision in place, and when that is the published revision, the draft's content and `workflow_state` are written into the very row the published pointer serves (live content corruption). The unconditional set is order-independent with respect to `NodeRevisionDefaultListener` (Task 2.3): that listener respects an already-set non-null value, so guard-first keeps `true`; node-listener-first sets `false`, which the guard then overrides.

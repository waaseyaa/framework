...
**Pointer-move guard reconciliation (carried from task 2.5's review).** `TransitionService` validates a forward draft's `publish` transition against the tip revision's own, real predecessor state (e.g. `review -> published`, or `draft -> published` after a `restore`) — the correct basis, since that is the actual edge the acting account requested and was granted permission for. `WorkflowPointerMoveGuard`, by contrast, only ever sees a before/after snapshot of the *published pointer itself*, which can legitimately be several transitions stale by the time a forward draft catches up and gets published (e.g. `archived -> [restore, no pointer move] -> draft -> [submit_for_review, no pointer move] -> review -> [publish, pointer moves NOW] -> published` — none of the intermediate hops touch the pointer, so the guard's "currently effective state" is still `archived` when the final publish fires). Re-deriving a literal `currentState -> newState` edge in that case looks for `archived -> published`, which was never meant to exist as a direct edge, and would wrongly deny an already-permission-checked, fully sanctioned publish.

Fix, scoped narrowly to the `publish` operation: when the **target** state is itself flagged `default_revision: true`, the guard validates against *any* transition that legally reaches that state (regardless of its declared `from`), instead of specifically from the stale pointer state; permission is resolved from whichever transition targets the state (in the reference `editorial` workflow, exactly one transition ever targets a given default-revision state, so this is unambiguous). This does not reopen the pointer-move bypass tasks 2.4/2.5 closed: a revision can only carry a `workflow_state` of `published`/`archived` in the first place because some earlier save legitimately reached it — either via `TransitionService::transition()` (this same call, moments earlier) or a raw save validated by `WorkflowStateGuard` (which requires an edge + permission just the same). The guard's job is to stop a *bypass* caller from promoting a revision that was never legitimately reached at all (e.g. a hand-rolled `setPublishedRevision()` call pointed at a `draft` revision) — not to re-litigate a promotion whose destination is structurally a valid publish target. A target that is NOT `default_revision: true`, or any non-`publish` pointer operation (`rollback`/`revert`), still goes through the original, stricter `currentState -> newState` edge check — exactly the scenario the guard exists to catch.

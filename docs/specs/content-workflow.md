...
**`WorkflowStateGuard::applyState()` mirrors the status rule** for raw saves: when the target state is `default_revision: false` and `EntityRepository::loadPublishedRevision()` returns a revision for the entity, `status` is copied from that published revision instead of following the target state's `published` flag. The guard fires on `EntityEvents::PRE_SAVE`, before the save that would even produce a new revision id — it has no way to orchestrate the save-then-`setPublishedRevision()` two-step, so a raw save into a `default_revision: true` state still cannot move the pointer; only `TransitionService` (or a direct, sanctioned repository call) does that. This is a documented, narrower guarantee than the service's, not a gap the guard silently papers over.

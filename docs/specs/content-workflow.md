...
Consequence worth knowing (accepted, matches the strict rule): with the shipped editorial edge set, content whose published pointer sits on an `archived` revision cannot be republished by promoting a fresh `published`-stamped forward draft — that is an `archived -> published` different-state move with no edge. Drupal core's editorial ships an `archived -> published` "Restore" edge for exactly this; adding one here is a workflow-config decision (site config or a future default-workflow amendment), not an engine change.

...
The ergonomic dev front door for a downstream app is the skeleton's **`composer dev`** — a cross-platform PHP launcher (`skeleton/bin/dev`, wired as `@php bin/dev` so Composer's own PHP runs it). It resolves the `frankenphp` binary to an **absolute path** (`FRANKENPHP_BIN` → known per-OS install locations via `Waaseyaa\Foundation\Runtime\FrankenPhpLocator` → `frankenphp` on PATH) and execs it in classic per-request mode on `127.0.0.1:8080` (loopback + non-privileged, so no privileged-port or auto-HTTPS prompt). It **never adds the FrankenPHP directory to `PATH`**, so the official Windows release's bundled OpenSSL-disabled `php.exe` cannot shadow system PHP and break Composer. (`composer dev` in the framework monorepo itself remains the `php -S`/`dev:php` path for framework development — the FrankenPHP launcher is the *skeleton/downstream* `composer dev`.) For the warm, concurrent worker runtime, run frankenphp directly against the committed `config/frankenphp/Caddyfile` (worker mode → `public/index.php`):

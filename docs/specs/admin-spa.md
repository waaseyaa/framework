...
2. **Pinned toolchain** — the hermetic child already refuses any runtime other than the `.nvmrc` pin (`AdminBuildToolchainPolicy`); the acceptance step independently refuses a `--node-runtime` whose major does not equal that pin, so a mis-pinned parent shell cannot slip an unpinned build through.
...
**The acceptance gate.** `bin/admin-dist-acceptance verify` (composer `check-admin-dist-manifest`, `tools/preflight-gates.json`, and blocking `ci/verify-gates`) re-derives every manifest claim from the committed bytes with no Node toolchain: the published tree digest and counts, source-signature parity with `dist.signature`, the Nuxt build identity in `_nuxt/builds/latest.json`, the marker roster digest, and each declared marker's presence in the served bundle. It is **additive to** `check-admin-dist-fresh`, which remains the authoritative D6 staleness gate and is not weakened, bypassed, or replaced: freshness answers "did source advance without a rebuild", acceptance answers "do the committed bytes match what the operation said it published".

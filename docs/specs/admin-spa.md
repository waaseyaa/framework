...
<!-- Spec reviewed 2026-05-25 - mission ai-observability-recent-runs-m5b-01KSEFT9 (#1415) WP02: AI observability runs SPA. New pages at `/ai/observability/runs` (list) and `/ai/observability/runs/[uuid]` (detail + replay). `useAiObservabilityRuns` composable: `{rows, total, page, perPage, filter, loading, error, fetchRuns(), setFilter(partial), setPage(n)}` — hits `GET /api/ai/observability/runs` with `page`, `per_page`, `pipeline`, `status`, `from`, `to` query params; `setFilter` resets page to 1. `useAiObservabilityRunDetail` composable: `{run, loading, error, fetchRun(uuid), replay(uuid)}` — hits `GET /api/ai/observability/runs/{uuid}` and `POST /api/ai/observability/runs/{uuid}/replay`. Components: `RunFilterBar.vue` (pipeline text + status select + from/to date inputs, emits `update:modelValue`), `RunListTable.vue` (props: rows/page/perPage/total, emits `page-change`), `RunSpanNode.vue` (recursive, honours `truncated: true` with no-children + label, bounded at server-side MAX_DEPTH=32). NavBuilder gains an "AI" section (using existing `nav_group_ai` i18n key) with a `/ai/observability/runs` link (`nav-ai-observability-runs` test-id) above the existing Operations section. 26 new `ai_runs_*` i18n keys in `en.json`. Vitest unit tests: `useAiObservabilityRuns.test.ts` (6 cases: empty start, fetch+populate, page param, pipeline filter, status filter, filter resets page) and `useAiObservabilityRunDetail.test.ts` (4 cases: null start, fetch+populate, replay POST, replay error). E2E skeleton `ai-observability-runs.spec.ts` (all tests skipped — deferred to CI with `nuxt dev`). -->

...
<!-- Spec reviewed 2026-08-22 - #2494 FrankenPHP worker mode includes auto_prepend_file once per process, not once per request. The repo `public/index.php` `$handler` therefore requires a repository-owned activator (`tests/Acceptance/FrankenPhpWorker/activate.php`) only when `WAASEYAA_FRANKENPHP_ACCEPTANCE` is exactly `worker-lane-v1`. The activator demands SAPI `frankenphp`, refuses environment-supplied paths and request-header activation, and fails closed if the probe fixture is missing. Production leaves the env unset. Skeleton / `make:public` stub / golden stay unchanged. -->

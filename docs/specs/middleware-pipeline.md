# Middleware Pipeline
<!-- Spec reviewed 2026-08-10 - #2327: FieldReadContextMiddleware obtains community scope from explicit request attributes, then the resolved session selector, then the authoritative active CommunityContextInterface. This keeps immutable access principals aligned with fixed-community storage/controllers without changing route/session precedence; inactive contexts remain unscoped. -->

...
<!-- Spec reviewed 2026-05-25 - mission ai-observability-recent-runs-m5b-01KSEFT9 (#1415) WP01: RunListReadModel aggregates span cost/token data from the `trace_span` supporting table via DatabaseInterface::select(); date-range filtering applied PHP-side since EntityRepositoryInterface::findBy() lacks range operators. RunDetailReadModel builds a recursive span tree (MAX_DEPTH=32, truncated:true sentinel at boundary); malformed JSON attributes are caught with \JsonException and replaced with empty array — not fatal. RunReplayService resolves the original trace label as pipeline ID, looks up the Pipeline config entity, creates a new Trace with status='queued', and dispatches via PipelineDispatcher (fire-and-forget async). All three implementations live in ai-observability (L5) implementing api (L4) contracts — correct downward cross-layer. -->

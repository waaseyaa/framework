...
<!-- Spec reviewed 2026-09-08 - #3040: `trace.label` is an Internal framework-owned field-read default. The observability provider's registered Trace metadata and activation preflight now consume the same central classification, without creating an account-facing read channel or changing the separately permissioned recent-invocation projection. -->
<!-- Spec reviewed 2026-08-25 - #2131: `pipeline.label` is a first-party config label with the same Public framework default as `media_type.label` and `menu.label`. Installing `waaseyaa/ai-pipeline` no longer leaves `pipeline|*|label` unclassified, so field-access activation preflight cannot hard-block a consumer that never uses pipelines. -->
<!-- Spec reviewed 2026-08-07 - #2304: `User.mail` remains Internal. A signed-in account's interactive profile UI does not receive generic field-read authority; it uses `UserSelfProfileReaderInterface`, whose audited implementation releases only `{name, mail}` for an authenticated immutable principal whose id matches the target User. Mismatched or anonymous actors fail before capability reservation. -->
...
<!-- Spec reviewed 2026-09-02 - #2786 B1: kernel-wired SchemaPresenter instances receive FieldSchemaAuthority over the one boot-scoped, manifest-fed FieldTypeManager; FieldTypeManager::default() is built-ins-only and limited to bare/isolated construction. The entity `revision` key remains an integer, readOnly, hidden-widget system property. No field-access decision changes. -->
<!-- Spec reviewed 2026-09-01 - #2786: SchemaPresenter access filtering now requires the subject entity whenever handler/account context is supplied; partial subject/principal context throws instead of returning an unfiltered schema. Structural field shapes, including item-level constraints for multi-value fields, come from FieldSchemaAuthority; the field-access decisions below are unchanged. -->
...
| `trace.label` | Internal | No account-facing role or channel | The label belongs to execution-observability state. Generic entity reads remain deny-by-default, and the separate recent-invocation projection continues to require `ai_observability.view_traces` and redact its bounded fields. |
...
`ResourceSerializer::serialize()` accepts paired `?EntityAccessHandler` +
`?AccountInterface`. `SchemaPresenter::present()` additionally requires its
`?EntityInterface` subject whenever that pair is present. A handler/account
pair with no subject throws `PartialAccessContextException`; it cannot silently
skip field filtering.

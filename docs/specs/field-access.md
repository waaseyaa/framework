...
<!-- Spec reviewed 2026-09-02 - #2786 review round: SchemaPresenter composes its FieldSchemaAuthority over the shared FieldTypeManager::default() registry (the same instance FieldServiceProvider binds), and emits the entity `revision` key as an integer, readOnly, hidden-widget system property so it is never offered as an editable field. No field-access decision changes. -->

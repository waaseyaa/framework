...
Validator, etc.) as the foundation libraries. Doctrine DBAL is the
established database layer; `waaseyaa/database-legacy` remains only as the
named (explicitly temporary) transitional bridge. Twig for SSR.
Vue 3 + TypeScript + Nuxt 3 for the admin SPA. PHPUnit 10.5 for tests.
...
| 2026-06-17 | Truth-up corrections (factual drift, no policy change): DIR-005 canonical-spec pointer changed from the SUPERSEDED `entity-storage-two-axis.md` to the live `revision-system-unified.md`; Policy Summary corrected "PHPUnit 11" → "PHPUnit 10.5" (matches root `composer.json`); Doctrine DBAL noted as the established DB layer (database-legacy retained as the named bridge), dropping the stale "in-flight via the doctrine-dbal-cutover mission" framing. The CI-matrix (Debian + PostgreSQL + `ci-template.yml`), `tools/bench/` harness, exception-policy automation, and `RELEASE.md`/release-signing mandates were RETAINED as-is and recorded as build-up work — author chose "build up to the charter," not amend down. | Author (Russell Jones) authorisation captured during the 2026-06-17 charter-review session via `AskUserQuestion` answers. Mission: `charter-truth-up-2026-06-01KVB6MW`. |

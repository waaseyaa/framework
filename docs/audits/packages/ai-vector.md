...
- **Consumers:**
  - Its own provider registers the lifecycle listeners in every kernel (FW-AIV-COMP-01). At the base, only Foundation `HttpKernel` registered them, and only under HTTP.
...
- **Disposition and owner:** resolved by #3139/#3155 (`d58d526ab`) (FW-AIV-COMP-01). Every consumer uses the kernel services' first binding of each interface. ai-vector always binds the storage but binds an embedding provider only when one is configured, so the storage and provider can come from different providers, while every consumer receives the same pair. Choosing a different storage backend belongs to #3140.
...
- **Residual risk:** none.
- **Next action:** none for this finding. Backend selection is owned by #3140.
...
- **Disposition and owner:** resolved by #3139/#3155 (`d58d526ab`) (FW-AIV-COMP-01). Outside HTTP, saves and deletes remove any existing vector without calling the provider, and `semantic:refresh` re-indexes.
...
- **Observed, with evidence:**
  ...
  - Traced in source at the base, then reproduced end to end through a real repository by `PostCommitVectorFailureTest`, which fails at the base and passes since FW-AIV-COMP-01.
...
- **Disposition and owner:** resolved by #3139/#3155 (`d58d526ab`) (FW-AIV-COMP-01). Post-commit vector storage failures are best-effort, logged once and never surfaced.
- **Dependencies:** none.
...
- **Persistence and execution:**
  ...
  - Reported versus durable outcome: embed-and-store failures are logged and swallowed (AIV-EXEC-001). At the base, delete-path failures aren't; they surface as a committed-side-effects failure of the entity mutation (AIV-EXEC-002). Since FW-AIV-COMP-01 they are logged and swallowed too.
...
- **Kernel and runtime:**
  ...
  - Boot failure: a misconfigured OpenAI credential throws at register time and fails closed (reviewed, acceptable). At the base it throws from the provider and again from `HttpKernel`'s listener wiring; since FW-AIV-COMP-01, only from the provider.
  ...
  - Repeated execution: at the base, the storage caches its schema check per instance and `SearchRouter` creates a new instance per request. Since FW-AIV-PERSIST-01 the storage has no schema check; since FW-AIV-COMP-01, search resolves the one kernel-bound instance.
- **Domain contracts:**
  ...
  - Events: POST_SAVE, POST_DELETE, `RevisionPointerMovedEvent` and REVISION_REVERTED. At the base they are registered only by `HttpKernel` (AIV-COMP-002), after the discovery and MCP read-cache listeners. Since FW-AIV-COMP-01, ai-vector's provider registers them at boot in every kernel.
  - Ordering and veto: these are notification events dispatched after commit, so they can't veto a mutation. A listener exception stops later listeners for the same event. At the base, ai-vector's delete paths can throw (AIV-EXEC-002); since FW-AIV-COMP-01 they don't.

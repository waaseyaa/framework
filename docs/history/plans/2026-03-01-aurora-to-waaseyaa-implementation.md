...
**Step 4: Verify no remaining `Aurora\` in PHP files (excluding docs/history/plans/)**
...
```bash
grep -r 'bin/aurora' . --include='*.php' --include='*.json' --include='*.md' --include='*.xml' -l | grep -v docs/history/plans/ | grep -v vendor/
```

If any hits, fix them:
```bash
find . -name '*.php' -not -path './vendor/*' -not -path './docs/history/plans/*' -exec sed -i 's|bin/aurora|bin/waaseyaa|g' {} +
```

**Step 7: Verify no remaining Aurora references in PHP files (excluding docs/history/plans/)**
...
grep -ri 'aurora' packages/ tests/ bin/ public/ --include='*.php' --include='*.json' --include='*.ts' --include='*.vue' | grep -v vendor/ | grep -v docs/history/plans/ | grep -v node_modules | grep -v package-lock
```
...
```bash
grep -ri 'aurora' . --include='*.php' --include='*.json' --include='*.xml' --include='*.md' --include='*.ts' --include='*.vue' | grep -v vendor/ | grep -v docs/history/plans/ | grep -v node_modules | grep -v package-lock | grep -v '.git/'
```

Only the design docs in `docs/history/plans/` and this implementation plan should contain "Aurora".

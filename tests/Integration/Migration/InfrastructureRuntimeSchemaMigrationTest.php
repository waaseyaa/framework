...
final class InfrastructureRuntimeSchemaMigrationTest extends TestCase
{
    ...
    public function package_declared_migrations_install_the_infrastructure_schema_family(): void
    {
        ...
        $packages = ['api', 'cache', 'publishing', 'state'];
        $paths = [];
        ...
        self::assertSame(4, $migrator->run($all)->count);

        $schema = new SchemaBuilder($connection);
        foreach (['_broadcast_log', '_broadcast_retained', 'cache_items', 'publishing_idempotency', 'state'] as $table) {

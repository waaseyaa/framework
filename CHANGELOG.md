...
- **mcp**: `McpServiceProvider::register()` now binds `Mcp\Bridge\ToolRegistryInterface`, `Mcp\Bridge\ToolExecutorInterface` (both backed by a single `AgentToolRegistryBridge` singleton), and `Mcp\Auth\McpAuthInterface` (default: empty-token `BearerTokenAuth`). The bridge wraps the framework-wide `Waaseyaa\AI\Tools\ToolRegistryInterface` so every `#[AsAgentTool]`-discovered tool — including M2's four bimaaji adapters — appears automatically over the new `/mcp` endpoint via `McpEndpoint`. WP02 caveat: the bridge takes `AccountInterface` at construction, so the binding currently uses a no-permission placeholder; `tools/list` works, `tools/call` returns the documented `forbidden` envelope until WP03 lands per-request account passthrough. `BimaajiMcpReadTest` integration test pins both behaviours. M3 WP02 of mission `bimaaji-mcp-bridge-01KS5VS8`.

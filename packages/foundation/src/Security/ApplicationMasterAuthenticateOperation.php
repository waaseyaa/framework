...
final readonly class ApplicationMasterAuthenticateOperation implements SecretConsumerInterface
{
    ...
    {
        return 'waaseyaa.application.master-authenticate.v1';
    }
    ...
    public function consume(#[\SensitiveParameter] string $bytes, string $version): ApplicationMasterAuthenticationTag
    {
        ...
        try {
            $digest = hash_hmac('sha256', $this->message, $key, true);
        } finally {
            sodium_memzero($key);
        }
    ...
    {
        throw new \LogicException('Application-master authenticate operations cannot be serialized.');
    }
}

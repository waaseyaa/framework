...
final class GenericAdminSurfaceHostSaveAdvisoryTest extends TestCase
{
    ...
    #[Test]
    public function if_match_header_does_not_replace_the_body_mutation_token(): void
    {
        $entity = new TestEntity(['id' => 1, 'title' => 'News'], 'article');
        $token = EntityMutationToken::issue('test', 'default', 'article', '1', 1);
        $entity->_hydrateMutationToken($token);
        $repository = $this->createStub(EntityRepositoryInterface::class);
        $repository->method('find')->willReturn($entity);
        $repository->method('save')->willThrowException(
            new \RuntimeException('Save must not run without a body mutation token.'),
        );

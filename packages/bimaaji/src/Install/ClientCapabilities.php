 ...
 * {@see Client\ClaudeClientTransformer} opens a per-skill file with YAML
 * frontmatter — so an unenforced contradiction is a silently wrong install,
...
final readonly class ClientCapabilities
{
     ...
     * @param bool $requiresFrontmatterAtByteZero Whether a per-skill file MUST open with a YAML frontmatter block at byte 0 for the client to discover it (true for Claude Code). {@see Client\ClaudeClientTransformer} derives its per-skill rendering from this flag, so it is load-bearing, not documentation. Rejected as a contradiction for a `SingleConsolidatedFile` client, which emits no per-skill file for it to govern.

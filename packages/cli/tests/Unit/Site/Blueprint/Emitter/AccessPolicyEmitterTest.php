...
final class AccessPolicyEmitterTest extends TestCase
{
    ...
    /**
     * #2848 convergence proof: `renderPolicyClass()` (the `make:policy`
     * entrypoint) and `emit()` apply the same renderer to the same
     * permission-only policies on an entity with no authorization inputs —
     * the generated decision logic is byte-identical aside from the manual-
     * scaffold provenance header.
     */
    #[Test]
    public function renderPolicyClassConvergesWithEmitForPermissionOnlyPolicies(): void
    {
        $viewPolicy = new BlueprintPolicy(
            'article_view',
            'article',
            BlueprintOperation::View,
            new BlueprintPolicyCondition(BlueprintConditionKind::Permission, permission: 'view article'),
        );
        $updatePolicy = new BlueprintPolicy(
            'article_update',
            'article',
            BlueprintOperation::Update,
            new BlueprintPolicyCondition(BlueprintConditionKind::Permission, permission: 'edit article'),
        );
        $manifest = $this->manifestWithBlueprint($this->blueprintWithPolicies([$viewPolicy, $updatePolicy]));

        $emission = new AccessPolicyEmitter()->emit($manifest->applicationBlueprint, $manifest);
        self::assertCount(1, $emission->artifacts);

        $policies = [$viewPolicy, $updatePolicy];
        usort($policies, static fn(BlueprintPolicy $left, BlueprintPolicy $right): int => strcmp($left->id, $right->id));

        $fromManual = new AccessPolicyEmitter()->renderPolicyClass(
            'article',
            ownerField: null,
            className: 'ArticlePolicy',
            policies: $policies,
            workflowBound: false,
        );

        self::assertStringContainsString(
            'Generated by Waaseyaa\\CLI\\make:policy (stdout scaffold — review before registering).',
            $fromManual,
        );
        self::assertStringNotContainsString('ApplicationBlueprintCompiler', $fromManual);
        self::assertSame(
            $this->stripPolicyProvenanceHeader($emission->artifacts[0]->content),
            $this->stripPolicyProvenanceHeader($fromManual),
        );

        [$namespace, $entityClass, $policyClassFromEmit] = $this->loadGeneratedPolicyWithEntity(
            $emission->artifacts[0]->content,
            $manifest,
        );
        $manualNamespace = $namespace . 'Manual';
        $policySourceManual = str_replace(
            ['namespace App\\Access;', 'App\\Entity\\Article'],
            ['namespace ' . $manualNamespace . '\\Access;', $namespace . '\\Entity\\Article'],
            $fromManual,
        );
        $dir = sys_get_temp_dir() . '/waaseyaa_access_policy_convergence_' . bin2hex(random_bytes(8));
        mkdir($dir, 0o700, true);
        file_put_contents($dir . '/ArticlePolicy.php', $policySourceManual);
        require $dir . '/ArticlePolicy.php';
        $policyClassFromManual = $manualNamespace . '\\Access\\ArticlePolicy';

        $handlerEmit = new EntityAccessHandler([new $policyClassFromEmit()]);
        $handlerManual = new EntityAccessHandler([new $policyClassFromManual()]);
        $viewer = AuthorizationPrincipalFactory::authenticated(1, permissions: ['view article']);
        $editor = AuthorizationPrincipalFactory::authenticated(1, permissions: ['edit article']);
        $anonymous = AuthorizationPrincipalFactory::anonymous();
        $entity = $this->sealedArticle($entityClass, authorId: 1, workflowState: 'draft');

        self::assertSame(
            $handlerEmit->check($entity, 'view', $viewer)->isAllowed(),
            $handlerManual->check($entity, 'view', $viewer)->isAllowed(),
        );
        self::assertSame(
            $handlerEmit->check($entity, 'update', $editor)->isAllowed(),
            $handlerManual->check($entity, 'update', $editor)->isAllowed(),
        );
        self::assertSame(
            $handlerEmit->check($entity, 'view', $anonymous)->isAllowed(),
            $handlerManual->check($entity, 'view', $anonymous)->isAllowed(),
        );
    }
    ...
    {
        return $this->blueprintWithPolicies([
            new BlueprintPolicy('bad_policy', 'article', $operation, $condition),
        ]);
    }
     ...
     */
    private function blueprintWithPolicies(array $policies): ApplicationBlueprint
    {
        $indexed = [];
    ...
    private function stripPolicyProvenanceHeader(string $source): string
    {
        return preg_replace(
            '/\/\*\*[\s\S]*?\*\/\s*(?=#\[PolicyAttribute)/',
            '',
            $source,
            1,
        ) ?? $source;

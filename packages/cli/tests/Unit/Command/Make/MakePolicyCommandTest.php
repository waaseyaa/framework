...
final class MakePolicyCommandTest extends TestCase
{
    ...
    #[Test]
    public function it_rejects_an_invalid_entity_machine_name(): void
    {
        $tester = $this->createTester();
        $tester->execute(['ContentPolicy', '--entity=bad entity']);

        $this->assertSame(2, $tester->getExitCode());
        $this->assertStringContainsString('Invalid entity', $tester->getStderr());
    }

    #[Test]
    public function it_defaults_to_the_blueprint_entity_class_convention_without_entity_class(): void
    {
        $tester = $this->createTester();
        $tester->execute(['ContentPolicy', '--entity=article', '--grant=view:view article']);

        $this->assertSame(0, $tester->getExitCode());
        $this->assertStringContainsString('assert($entity instanceof \\App\\Entity\\Article);', $tester->getStdout());
    }

    #[Test]
    public function it_escapes_single_quotes_in_grant_permissions(): void
    {
        $tester = $this->createTester();
        $tester->execute(['ContentPolicy', '--entity=article', "--grant=view:can't peek"]);

        $this->assertSame(0, $tester->getExitCode());
        $this->assertStringContainsString("hasPermission('can\\'t peek')", $tester->getStdout());
        $this->assertStringNotContainsString("hasPermission('can't peek')", $tester->getStdout());
    }

    #[Test]
    public function manual_output_carries_make_policy_provenance_not_blueprint_compiler(): void
    {
        $tester = $this->createTester();
        $tester->execute(['ContentPolicy', '--entity=article']);
        ...
        $this->assertSame(0, $tester->getExitCode());
        $output = $tester->getStdout();
        $this->assertStringContainsString('Generated by Waaseyaa\\CLI\\make:policy (stdout scaffold — review before registering).', $output);
        $this->assertStringNotContainsString('ApplicationBlueprintCompiler', $output);
    }

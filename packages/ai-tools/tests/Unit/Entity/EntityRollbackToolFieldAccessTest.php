...
/**
 * R4 PR1 WP2 (security, audit-remediation batch 2026-07-03): entity.rollback
 * writes the WHOLE target-revision row back over the current entity via
 * EntityRepository::rollback(), so restoring an old revision can silently
 * re-apply a privileged field the calling account could never set through
 * entity.update directly (e.g. a `roles` field gated by 'administer users').
 * The tool must gate the fields the restore would CHANGE the same way
 * entity.update gates its values payload.
 */
#[CoversClass(EntityRollbackTool::class)]
final class EntityRollbackToolFieldAccessTest extends TestCase
{
    ...
    private EntityRollbackTool $tool;
    ...
    protected function setUp(): void
    {
        ...
        $this->tool = new EntityRollbackTool(new SingleTypeEntityTypeManager($this->entityType, $this->repo));
    ...
    #[Test]
    public function non_admin_cannot_restore_a_revision_that_would_re_grant_privileged_roles(): void
    {
        $this->seedPrivilegedHistory();

        $result = $this->tool->execute(
            ['entity_type' => 'test_revisionable', 'id' => '1', 'target_revision_id' => 1],
            $this->nonAdmin(),
        );

        self::assertTrue($result->isError, 'restoring a privileged-field change must be refused');
    ...
    #[Test]
    public function admin_can_restore_a_revision_that_changes_privileged_roles(): void
    {
        $this->seedPrivilegedHistory();
    ...
    #[Test]
    public function admin_can_restore_across_a_credential_field_rotation(): void
    {
        ...
        self::assertFalse($result->isError, 'a credential-only difference must not block the restore: ' . ($result->summary ?? ''));
        $reloaded = $this->repo->find('1');
        \assert($reloaded instanceof TestRevisionableEntity);
    ...
    #[Test]
    public function non_admin_can_restore_a_revision_that_only_changes_non_privileged_fields(): void
    {
        // Both revisions carry the same roles — the restore only changes `title`.

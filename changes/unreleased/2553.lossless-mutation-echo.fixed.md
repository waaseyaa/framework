- **A JSON:API mutation can now echo the lossless editor projection, so a client
  ...
  changes. The framework's own admin SPA does not opt in yet and must re-read
  before editing; adopting it there also requires adopting `?workingCopy=1`,
  which changes which revision the admin edits and is a separate decision.

- **Added — `waaseyaa/ai-development`, the `require-dev` metapackage that
  ...
  **The bootstrap-window path repository names its own version instead of
  ...
  fixture, as the negative control. **The Windows lane never shared this
  defect** and is untouched: `tests/ReferenceConsumer/prepare.php` already
  localizes every package at `dev-main`, which the manifests' own
  `branch-alias` maps to `0.1.x-dev`.

- **Added — `waaseyaa/ai-development`, the `require-dev` metapackage that
  ...
  **The same blindness, one layer down, also closed.** RR002's roster is parsed
  out of the workflow with a pattern that recognised exactly one spelling of a
  matrix row, so `remote: "beta"` or a bare `remote: beta` parsed to nothing and
  the target vanished. RR000's blind-pass guard could not catch it: it fires only
  when the roster is entirely empty, so the seventy-odd parseable rows kept it
  satisfied while one edited row disappeared unseen — a guard against total
  parse failure is powerless against a partial one, and partial is the shape
  that actually occurs when someone edits a single row. The symmetry is worth
  recording, because it is the same mistake twice: RR001's roster could not see
  a package no manifest requires, and RR002's could not see a target whose YAML
  uses a different quote character. Both are *the input is structurally
  incapable of representing the thing being gated*, so both are fixed at the
  input rather than by widening a pattern to admit one more reported case.
  Anything that looks like a matrix target and cannot be read is now refused
  (**RR003**), and matrix *structure* the gate cannot understand — absent,
  duplicated, empty, entry-less, or an entry naming no target — is refused too
  (**RR004**) instead of silently shrinking the roster.
  The parser stays dependency-free on evidence rather than habit: this gate runs
  in three places (`release-cut.yml`'s pre-tag step, `split.yml`'s
  `verify-require-parity`, and the architecture test that executes it), none of
  them provisions a YAML parser, no workflow installs PyYAML and no tooling
  imports it. Relying on `import yaml` happening to succeed would be the same
  undeclared-property fragility being fixed here.
  The regression fixture mixes all three scalar forms in one matrix with only
  the parseable one provisioned, because a fixture containing only the
  unreadable form would fail for the wrong reason and prove nothing about
  concealment: the previous parser reports success on it while two unreachable
  targets hide behind the survivor, and the corrected one names both.

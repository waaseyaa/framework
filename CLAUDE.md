...
- Composer policy is codified and gated via `bin/check-composer-policy`:
  ...
  - the local AI-development plane stays out of production (CP009, ADR-022 D-1.2/D-3.0/D-10): the gate computes the transitive `waaseyaa/*` **runtime** require closure of `composer.json`, `packages/core`, `packages/cms`, and `packages/full`, and fails if `waaseyaa/ai-development` or `waaseyaa/ai-agent` (which homes `LocalOperatorPrincipal`) appears in one, or if `waaseyaa/ai-development` appears in any first-party `require` block. Seeded positive controls: `tests/Architecture/DevelopmentPlaneClosureGateTest.php`
...
| `packages/ai-development/*` | — (require-dev development metapackage) | `docs/adr/022-ai-development-package-and-local-operator-trust-boundary.md` |
...
- After design review, if the change affects a spec you intentionally leave
  ...
  spec is affected, no trailer is required, and one is

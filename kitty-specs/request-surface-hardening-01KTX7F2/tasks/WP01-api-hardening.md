...
- 2026-06-12T06:58:01Z – claude:fable-5:reviewer:reviewer – shell_pid=25272 – Review passed: contract clauses 1-17 verified against diff 79562e180. Discovery: anonymous/null account -> envelope-only (meta+links.self), zero type ids in body; authenticated -> discoverable types only; discoverable:false absent for all callers; EntityTypeInterface untouched; duck-typed read guarded with method_exists open-default; NFR-001 holds (isAuthenticated once + accessor per type, no policies/storage). Denied-as-404: single notFoundDocument factory, literal moved verbatim; same-id-two-worlds byte-identity pin + 404 status equality + no-denial-trace scan in JsonApiControllerDeniedNotFoundTest; mutation 403s (store/update/destroy/field-edit) untouched in code and tests. Known deviation accepted: JsonApiAccessIntegrationTest (not owned) - exactly the two denied-show assertions moved 403->404 (old-oracle pins); all mutation 403 assertions intact. No foundation/BuiltinRouteRegistrar changes; adjacent enumeration routes untouched. Gates run by reviewer: phpunit api+entity+Phase7 1107 tests OK, phpstan clean, cs-check clean, check-dead-code clean, check-package-layers OK.
- 2026-06-12T07:39:33Z – claude:fable-5:reviewer:reviewer – shell_pid=25272 – Done override: Mission squash-merged to main as 0f0a7bf2a

...
14. **No-expectation invariance** (superseded 2026-07-01, issue C-22): originally, a PATCH without `data.meta.expected_revision_id` (or without `data.meta` at all) followed the legacy `getStorage()->save()` path byte-identically — same checks, same responses, same events (FR-003/SC-003). C-22 WP3 unified `JsonApiController::update()` onto a single persistence engine: a no-expectation PATCH now persists through the SAME `getRepository()->save($entity)` pipeline as an expectation-stated PATCH (clause 11) — it simply omits the `SaveContext::withExpectedRevisionId()` guard, so no `RevisionConflictException` can surface. **Consequence:** a no-expectation PATCH on a revisionable type now cuts a new revision (previously it did not); the checks, responses, and events are otherwise unchanged. There is no separate "legacy path" once C-22 completes — see `docs/specs/entity-system.md` and `docs/notes/c22-consumer-inventory.md`.

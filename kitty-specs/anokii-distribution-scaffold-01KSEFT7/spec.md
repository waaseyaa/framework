...
- Author `.kittify/charter/charter.md` codifying:
  ...
  - **Indigenous-language translation pipeline as product layer** — extraction tooling → `translation_string` entity (mirrors two-axis storage) → contributor dashboard → `translation_review` workflow → glossary entity → per-Nation override layer. Pilot scope: English ↔ Anishinaabemowin (southern + northern Ojibwe dialects), 20–30 glossary terms co-authored with a language keeper. Pilot Nations: Sagamok-first (Russell's home Nation; OIATC already on Waaseyaa) with Sheguiandah as second. Final Nation selection deferred to the language-keeper engagement moment.
...
- **OIATC stewards channel involvement timing.** Anokii's charter (WP02) mentions OIATC stewards as the Nation-level governance channel referenced by framework DIR-008 (license-change amendment process). OIATC formal engagement is not a WP02 blocker (charter codification predates formal endorsement), but WP02 implementer notes the engagement as a future Anokii mission so it does not get lost.
...
- **Translation pipeline pilot Nations:** Sagamok-first (Russell's home Nation; OIATC already on Waaseyaa) with Sheguiandah as the second Nation. Final Nation selection deferred to the language-keeper engagement moment only.
...
- A follow-up Anokii mission will file the OIATC stewards engagement letter / formal endorsement track.
